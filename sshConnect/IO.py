...
class writer(object):
    ...
    def alter_pass(self,js,ip,password):
        for i in js['server']:
            if i['ip'] == ip:
                i['password'] = password
        logger.info("修改%s的正确密码为%s"%(ip,password))
    ...
    def _retry_add(self, js, ip, array):
        ...
        logger.info("增加%s的重试密码%s"%(ip,array))
        return json.dumps(js)
        ...
        for i in js['server']:
            if i['ip'] == ip:
                for j in i['retry']:
                    if j == delt:
                        i['retry'].remove(j)
        logger.info("删除%s的重试密码%s"%(ip,delt))
        return json.dumps(js)
    ...
    def csv_write_head(self, filename):
        ...
        head_infos = ["IP", "登录用户名","密码","操作系统类型","其他登录用户","当前登录用户","CPU个数","内存使用情况"]
